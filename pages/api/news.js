...
const GNEWS_QUERIES = {
  ...
  politics: {
    endpoint: "search",
    params:
      "q=política OR governo OR eleição OR congresso OR ministério&lang=pt&country=br&nullable=image&sortby=publishedAt",
  },
  business: {
    endpoint: "top-headlines",
  ...
  sports: {
    endpoint: "search",
    params:
      "q=futebol OR jogo OR resultado OR gol OR campeonato OR brasileirao OR copa&lang=pt&country=br&nullable=image&sortby=publishedAt",
  },
};
...
function processGNewsArticle(article, index) {
  ...
  const sourceName =
    article.source?.name || extractDomain(article.url) || "Fonte Desconhecida";
...
async function fetchNews(category = "home", limit = 20) {
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; OnlyDans/1.0; +https://onlydans.vercel.app)",
      },
    });
    ...
    console.log(
      `Successfully processed ${news.length} news items for category: ${category}`,
    );
